...
*
*  CameraShip()				initialize the ship so that it is located at the
...
class CameraShip {
  ...
  double speed;
public:
  CameraShip(Point initialPosition = Point(0, 0, 0));
...
inline CameraShip::CameraShip(Point initialPosition):
...
inline void CameraShip::pitch(double angle)
{
  forward = unit(forward * cos(angle) + up * sin(angle));
  up = right.cross(forward);
}

inline void CameraShip::roll(double angle)
{
  right = unit(right * cos(angle) + up * sin(angle));
  up = right.cross(forward);
}

inline void CameraShip::yaw(double angle)
{
